...
{
public:
	PointCloudStreamer(pcl::Grabber &capture, bool enableServer = false, bool enableClient = false, bool enableVis = true) :capture_(capture), enableClient_(enableClient), enableServer_(enableServer), enableVis_(enableVis),frameid(0),stepFrame_(false)
	{
		exit_ = false;
	//	world_.resize(height*width*2);
		if (enableVis_)
			initVis();
		if (enableServer_)
			initServer();
			...
			while (!exit_&&!cloud_viewer_.wasStopped()/*&&!image_viewer_.wasStopped()*/)
			{
				...
				if (has_data)
				{
					...
					if (enableVis_)
					{
						...
						cloud_viewer_.addPointCloud(world_.makeShared(),"world_");
...
private:
	...
	bool ransacFusion()
	{
		...
		std::cout<<"iter: "<<iter<<"with "<<best_inlier_n<<" of "<<valid_count<<" finish!!"<<std::endl;
